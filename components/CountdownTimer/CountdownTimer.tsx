...
const CountdownTimer: React.FC<P> = ({ duration, breathingMethod }) => {
    ...
    const steps = Object.keys(breathingMethod);
    const breathIntervals = Object.values(breathingMethod);
    const intervalBreakpoint = useRef(duration - breathIntervals[0]);
    ...
    const reset = () => {
        ...
        intervalBreakpoint.current = duration - breathIntervals[0] + 1;
    ...
    return (
        <div className="flex flex-col items-center md:w-2/3">
            ...
            <p className="text-3xl font-light capitalize">{steps[intervalPosition.current]}</p>
            { breathIntervals && <p className="text-lg font-light">...{currentCount - intervalBreakpoint.current + 1}</p>}
            <div>
